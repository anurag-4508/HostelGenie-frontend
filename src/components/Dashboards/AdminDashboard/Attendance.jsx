...
function Attendance() {
  const getALL = async () => {
    setProgress(30);
    const marked = await fetch("https://hostelgenie.onrender.com/api/attendance/getHostelAttendance", {
      method: "POST",
      headers: {
  ...
  const markAttendance = async (id, isPresent) => {
    const data = await fetch(`https://hostelgenie.onrender.com/api/attendance/mark`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
